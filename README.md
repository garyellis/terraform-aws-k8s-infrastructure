...
| etcd\_controlplane\_root\_block\_device | The stacked etcdcontrolplane nodes root ebs block device config | `list(map(string))` | <pre>[<br>  {<br>    "volume_size": 40<br>  }<br>]</pre> | no |
...
| etcd\_controlplane\_worker\_root\_block\_device | The stacked etcd/controlplane/worker nodes root ebs block device config | `list(map(string))` | <pre>[<br>  {<br>    "volume_size": 60<br>  }<br>]</pre> | no |
...
| etcd\_root\_block\_device | The etcd nodes root ebs block device config | `list(map(string))` | <pre>[<br>  {<br>    "volume_size": 40<br>  }<br>]</pre> | no |
...
| userdata | the ec2 instance userdata script. Overrides the default userdata script | `string` | `""` | no |
...
| worker\_root\_block\_device | The worker nodes root ebs block device config | `list(map(string))` | <pre>[<br>  {<br>    "volume_size": 40<br>  }<br>]</pre> | no |
